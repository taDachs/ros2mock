...
class ServiceVerb(VerbExtension):
    ...
    def add_arguments(self, parser, cli_name):
        arg = parser.add_argument(
            "service_name", help="Name of the ROS service to mock to (e.g. '/add_two_ints')"
        )
        arg.completer = ServiceNameCompleter(include_hidden_services_key="include_hidden_services")
        arg = parser.add_argument(
            "service_type", help="Type of the ROS service (e.g. 'std_srvs/srv/Empty')"
        )
        ...
        arg.completer = ServiceResponseCompleter(service_type_key="service_type")
        arg = parser.add_argument(
            "--editor", action="store_true", help="If set, opens the editor on response"
        )
        arg = parser.add_argument(
            "--timeout",
            type=float,
    ...
    def main(self, *, args):
        return mocker(args.service_type, args.service_name, args.values, args.editor, args.timeout)


def mocker(service_type: str, service_name: str, values: str, editor: bool, timeout: float | None):
    try:
